...
public:
	...
	void addElementIfNotEmpty(const std::string &key, const std::string &value);
...
inline void JSONMapWriter::addElementIfNotEmpty(const std::string &key, const std::string &value) {
	if (value.size() > 0 && writeKeyAndColon(key)) {
		writeString(value);
	}
}
